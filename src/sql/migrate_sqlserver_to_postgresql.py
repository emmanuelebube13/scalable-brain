...
class SQLServerConnection:
    ...
    def __init__(
        self, host: str, user: str, password: str, database: str, port: int = 1433
    ):
        self.host = host
        self.user = user
    ...
    def connect(self):
        ...
        try:
            ...
            logger.info(
                f"Connected to SQL Server: {self.host}:{self.port}/{self.database}"
            )
            return self.conn
            ...
            for i in range(0, len(rows), batch_size):
                ...
                for row in batch:
                    try:
                        ...
                        logger.warning(
                            f"Error inserting row into {table_name}: {e}, skipping row"
                        )

                self.conn.commit()
                logger.info(
                    f"Inserted {min(batch_size, len(batch))} rows into {table_name}"
                )
...
def verify_connections(
    sql_conn: SQLServerConnection, pg_conn: PostgreSQLConnection
) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate SQL Server database to PostgreSQL"
    )
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(
            f"Source: SQL Server {config['sql_server_host']}:{config['sql_server_port']}/{config['sql_server_db']}"
        )
        logger.info(
            f"Target: PostgreSQL {config['pg_host']}:{config['pg_port']}/{config['pg_db']}"
        )
