...
Usage: python -m src.scheduler.orchestrator           # evaluate triggers + maybe retrain
       python -m src.scheduler.orchestrator --force   # force a run (ignore triggers)
"""
...
from src.scheduler import triggers as TR
...
def _incumbent() -> Dict[str, Any]:
    ...
    * ``"absent"`` — genuinely nothing published anywhere; only here is fail-open correct
    ...
    from src.serializer.serialize import MODEL_PREFIX, POINTER_KEY
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        from src.serializer.publish_model_set import POINTER_KEY as SET_POINTER
...
def _gatekeeper_metrics() -> Dict[str, Any]:
    ...
    """
    try:
        from src.gatekeeper import train as G
...
def _promote_gatekeeper() -> Dict[str, Any]:
    ...
    """
    from src.gatekeeper.promote import ProposedBundleInvalid, promote_proposed
    from src.serializer import publish_gatekeeper as PG
...
def _default_promote(candidate: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    """
    from src.serializer import serialize as S
    ...
    if os.environ.get("MODEL_SET_AUTOPUBLISH", "false").strip().lower() != "true":
        logger.info(
            ...
            "run `python -m src.serializer.publish_model_set` to publish it"
        )
