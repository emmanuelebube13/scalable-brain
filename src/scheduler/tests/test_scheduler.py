...
from src.scheduler import orchestrator as O
...
def test_gatekeeper_autopromote_is_opt_in(monkeypatch):
    ...
    monkeypatch.setattr("src.gatekeeper.promote.promote_proposed", _boom)
...
def test_gatekeeper_autopromote_enabled_runs_the_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.gatekeeper.promote.promote_proposed", lambda d: {"model_path": "m"}
    )
    monkeypatch.setattr(
        "src.serializer.publish_gatekeeper.publish", lambda: {"version": "gk-1"}
    )
...
def test_gatekeeper_publish_refusal_does_not_fail_the_promote(monkeypatch):
    ...
    monkeypatch.setattr("src.serializer.publish_gatekeeper.publish", _refuse)
