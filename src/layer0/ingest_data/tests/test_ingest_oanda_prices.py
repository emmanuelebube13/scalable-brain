...
def test_fetch_candles_window_price_param() -> None:
    client = MagicMock()
    # default price
    fetch_candles_window(client, "EUR_USD", "H1", datetime(2026,1,1), datetime(2026,1,2))
    called_request = client.request.call_args[0][0]
    assert called_request.params["price"] == CONFIG.OANDA_PRICE
    
    # overridden price
    fetch_candles_window(client, "EUR_USD", "H1", datetime(2026,1,1), datetime(2026,1,2), price="MBA")
    called_request = client.request.call_args[0][0]
