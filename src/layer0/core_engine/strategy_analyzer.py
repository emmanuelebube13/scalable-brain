...
class StrategyAnalyzer:
    ...
    def analyze(
        self, result: BacktestResult, initial_capital: float = 100000.0
    ) -> StrategyMetrics:
        """
        ...
        if not result.equity_curve.empty:
            metrics.max_drawdown, metrics.max_drawdown_pct = (
                self._calculate_max_drawdown(result.equity_curve)
            )
        ...
        # Qualification check
        metrics.qualified, metrics.qualification_reasons = self._check_qualification(
            metrics
        )
    ...
    def _calculate_annualized_metrics(
        ...
        avg_bar_duration = (equity_curve.index[-1] - equity_curve.index[0]) / len(
            equity_curve
        )
    ...
    def _calculate_sharpe_ratio(
        self, equity_curve: pd.Series, annualized_volatility: float
    ) -> float:
        """
    ...
    def _check_qualification(self, metrics: StrategyMetrics) -> Tuple[bool, List[str]]:
        ...
        if metrics.total_trades < self.MIN_TRADES:
            qualified = False
            reasons.append(
                f"Insufficient trades: {metrics.total_trades} < {self.MIN_TRADES}"
            )

        if metrics.expectancy_r < self.MIN_EXPECTANCY_R:
            qualified = False
            reasons.append(
                f"Expectancy too low: {metrics.expectancy_r:.3f}R < {self.MIN_EXPECTANCY_R}R"
            )

        if metrics.profit_factor < self.MIN_PROFIT_FACTOR:
            qualified = False
            reasons.append(
                f"Profit factor too low: {metrics.profit_factor:.3f} < {self.MIN_PROFIT_FACTOR}"
            )

        if metrics.win_rate < self.MIN_WIN_RATE:
            qualified = False
            reasons.append(
                f"Win rate too low: {metrics.win_rate:.2%} < {self.MIN_WIN_RATE:.2%}"
            )

        if metrics.max_consecutive_losses > self.MAX_CONSECUTIVE_LOSSES:
            qualified = False
            reasons.append(
                f"Too many consecutive losses: {metrics.max_consecutive_losses} > {self.MAX_CONSECUTIVE_LOSSES}"
            )

        if metrics.max_drawdown_pct > self.MAX_DRAWDOWN_PCT:
            qualified = False
            reasons.append(
                f"Drawdown too high: {metrics.max_drawdown_pct:.2%} > {self.MAX_DRAWDOWN_PCT:.2%}"
            )
