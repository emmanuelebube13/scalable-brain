...
from ..data_access.indicators import calculate_pips, get_pip_value
...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        if len(df) == 0:
            ...
            return BacktestResult(
                strategy_name=strategy.config.name, asset=asset, granularity=granularity
            )

        # Run simulation
        trades = self._simulate_trades(strategy, df, signals, asset, granularity)
    ...
    def _simulate_trades(
        ...
        for i in range(len(df)):
            ...
            if current_position is None and signal != 0:
                ...
                if strategy.config.volatility_filter:
                    if not strategy.check_volatility_filter(
        ...
        if current_position is not None and current_position.exit_time is None:
            ...
            current_position.pnl = self._apply_friction(
                self._pnl_to_dollars(price_diff, asset)
            )
    ...
    def _check_exit(
        ...
        if not exited:
            ...
            max_bars = (
                strategy.config.max_bars_hold if hasattr(strategy, "config") else 50
            )
    ...
    def _build_equity_curve(self, trades: List[Trade], df: pd.DataFrame) -> pd.Series:
        ...
        for i in range(1, len(df)):
            ...
            if (
                ...
                and open_trade.exit_time is not None
                and open_trade.exit_time <= timestamp
            ):
