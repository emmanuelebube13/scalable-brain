...
class MultiTimeframeEngine:
    ...
    def validate_signal(
        ...
        if self.require_macro_alignment and macro_df is not None:
            macro_aligned = self._check_macro_alignment(primary_signal, macro_df)
    ...
    def _check_confirmation_alignment(
        self, primary_signal: Signal, confirmation_df: pd.DataFrame
    ) -> bool:
        """
    ...
    def _check_macro_alignment(
        self, primary_signal: Signal, macro_df: pd.DataFrame
    ) -> bool:
        """
    ...
    def _calculate_confluence_score(
        self, primary_signal: Signal, confirmation_aligned: bool, macro_aligned: bool
    ) -> float:
        """
    ...
    def align_timeframes(
        self, primary_df: pd.DataFrame, higher_df: pd.DataFrame, higher_granularity: str
    ) -> pd.DataFrame:
        """
            ...
            for col in higher_df.columns:
                if col not in ["Open", "High", "Low", "Close", "Volume"]:
                    ...
                    for i, timestamp in enumerate(result.index):
                        ...
                        valid_higher = higher_df_shifted[
                            higher_df_shifted.index <= timestamp
                        ]
                        if len(valid_higher) > 0:
                            result.loc[timestamp, f"{higher_granularity}_{col}"] = (
                                valid_higher[col].iloc[-1]
                            )

            # Forward fill any gaps
            higher_cols = [
                c for c in result.columns if c.startswith(f"{higher_granularity}_")
            ]
    ...
    def generate_mtf_signals(
        ...
        primary_df = strategy.calculate_indicators(
            primary_df.copy(), asset, primary_gran
        )
        ...
        for timestamp, signal_value in signals.items():
            ...
            signal = Signal(
                ...
                price=row["Close"],
                stop_loss=(
                    row["Close"] - row["ATR"] * 1.5
                    if signal_value == 1
                    else row["Close"] + row["ATR"] * 1.5
                ),
                take_profit=(
                    row["Close"] + row["ATR"] * 2.5
                    if signal_value == 1
                    else row["Close"] - row["ATR"] * 2.5
                ),
                confidence=0.5,
            )
