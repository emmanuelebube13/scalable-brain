...
def resample_ohlcv(
    df: pd.DataFrame, target_granularity: str, source_granularity: str = None
) -> pd.DataFrame:
    """
...
def merge_timeframes(
    primary_df: pd.DataFrame, higher_df: pd.DataFrame, higher_granularity: str
) -> pd.DataFrame:
    """
...
def detect_regime(
    df: pd.DataFrame, adx_threshold: float = 25.0, bb_width_threshold: float = 0.02
) -> pd.Series:
    """
...
def monte_carlo_simulation(
    returns: List[float], n_simulations: int = 1000, n_trades: int = 100
) -> Dict[str, float]:
    """
...
def print_trade_summary(trades: List, n: int = 10):
    ...
    print("-" * 80)
    print(
        f"{'Entry Time':<20} {'Dir':<4} {'Entry':<10} {'Exit':<10} {'P&L':<10} {'R':<6}"
    )
    print("-" * 80)
    ...
    for trade in trades[-n:]:
        ...
        print(
            f"{str(trade.entry_time):<20} {direction:<4} {trade.entry_price:.5f} "
...
def get_strategy_family(strategy_name: str) -> str:
    ...
    if "trend" in name_lower or "ema" in name_lower or "donchian" in name_lower:
        ...
        "range" in name_lower or "bollinger" in name_lower or "stochastic" in name_lower
    ):
...
def compare_strategies(
    results: List[Dict], metric: str = "expectancy_r"
) -> pd.DataFrame:
    """
