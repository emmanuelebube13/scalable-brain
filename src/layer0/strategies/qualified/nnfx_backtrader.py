...
def _calc_stc_c1(close: np.ndarray, fast: int, slow: int, cycle: int) -> np.ndarray:
    ...
    for t in range(len(close)):
        ...
        pf[t] = (
            pf[t - 1] + 0.5 * (f1[t] - (pf[t - 1] if t > 0 else 0)) if t > 0 else f1[t]
        )
    ...
    for t in range(len(close)):
        ...
        stc[t] = (
            stc[t - 1] + 0.5 * (f2[t] - (stc[t - 1] if t > 0 else 0))
            if t > 0
            else f2[t]
        )
...
class NnfxBacktrader(StrategyV2):
    ...
    def generate_orders(
        ...
        vol_f = _calc_damiani(
            ...
            low,
            close,
            self.DAMIANI_ATR_FAST,
            self.DAMIANI_ATR_SLOW,
            self.DAMIANI_STD_FAST,
            self.DAMIANI_STD_SLOW,
        )
