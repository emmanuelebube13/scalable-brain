...
from ...data_access.indicators import atr, detect_swing_points
...
class SupportResistanceStrategy(StrategyBase):
    ...
    def _find_support_resistance_levels(
        ...
        swing_highs, swing_lows = detect_swing_points(
            df["High"], df["Low"], self.swing_period
        )
        ...
        def cluster_levels(levels: np.ndarray, threshold: float) -> List[float]:
            ...
            for price in levels[1:]:
                if (
                    abs(price - np.mean(current_cluster)) / np.mean(current_cluster)
                    <= threshold
                ):
    ...
    def _count_level_touches(
        self, df: pd.DataFrame, level: float, is_support: bool
    ) -> int:
        """
        ...
        if is_support:
            touches = (df["Low"] >= level - threshold) & (
                df["Low"] <= level + threshold
            )
        else:
            touches = (df["High"] >= level - threshold) & (
                df["High"] <= level + threshold
            )
    ...
    def calculate_indicators(
        ...
        for i in range(level_lookback, len(df)):
            ...
            support_levels, resistance_levels = self._find_support_resistance_levels(
                window
            )
