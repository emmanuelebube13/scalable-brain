...
from ...data_access.indicators import atr, donchian_channel, adx, ema
...
class VCPBreakoutStrategy(StrategyBase):
    ...
    def calculate_indicators(
        ...
        df["Squeeze"] = (
            (df["VCI"] < self.contraction_threshold).fillna(False).astype(bool)
        )

        # Contraction duration
        df["Squeeze_Duration"] = (
            df["Squeeze"]
            .groupby((df["Squeeze"] != df["Squeeze"].shift()).cumsum())
            .cumcount()
            + 1
        )
        ...
        # Breakout signals
        df["Breakout_Up"] = (
            (df["Close"] > df["DC_Upper"].shift(1)).fillna(False).astype(bool)
        )
        df["Breakout_Down"] = (
            (df["Close"] < df["DC_Lower"].shift(1)).fillna(False).astype(bool)
        )
