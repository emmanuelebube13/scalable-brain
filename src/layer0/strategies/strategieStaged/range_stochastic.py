...
class RangeStochastic_Divergence(RangeStochasticStrategy):
    ...
    def _detect_bullish_divergence(
        ...
        stoch_low = (
            df["Stoch_K"].rolling(window=lookback, center=True).min() == df["Stoch_K"]
        )
    ...
    def _detect_bearish_divergence(
        ...
        price_high = (
            df["High"].rolling(window=lookback, center=True).max() == df["High"]
        )

        # Find Stochastic highs at same points
        stoch_high = (
            df["Stoch_K"].rolling(window=lookback, center=True).max() == df["Stoch_K"]
        )
