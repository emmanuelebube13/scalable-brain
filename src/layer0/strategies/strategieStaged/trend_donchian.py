...
from ...data_access.indicators import donchian_channel, adx, atr
...
class TrendDonchianStrategy(StrategyBase):
    ...
    def calculate_indicators(
        ...
        df["DC_Squeeze"] = df["DC_Width"] < df["DC_Width"].rolling(window=50).quantile(
            0.2
        )

        # Channel expansion (breakout potential)
        df["DC_Expansion"] = df["DC_Width"] > df["DC_Width"].rolling(
            window=50
        ).quantile(0.8)
