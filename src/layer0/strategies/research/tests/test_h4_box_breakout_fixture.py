...
test_week_one_long_matches_hand_computed_arithmetic
    §4 buffer B = 20 + 1.0 = 21 pips · E_long = box_high + B ·
    §6 stop = box_low with no buffer · §7 TP_k = E_long + k x H at 0.25 each ·
    §4 expires_after_bars = 29 · §6 no breakeven, no trail.
...
PIP = 0.01
BUFFER = 21 * PIP  # 20-pip noise buffer + 1.0-pip spread proxy (§4/§8)
...
#              expires_after_bars = 29 in §4. Bar 6 is BOX #1, a deliberately round
...
def test_bars_are_well_formed() -> None:
    ...
    # arithmetic behind expires_after_bars = 29 (§4).
...
def test_week_one_long_matches_hand_computed_arithmetic(orders: List) -> None:
    ...
    Box bar = 2024-01-07 21:00 (Sunday), High 185.00 / Low 184.00 / Close 184.50
      ...
      B        = (20 + 1.0) pips x 0.01         = 0.21      (§4/§8)

      E_long   = box_high + B = 185.00 + 0.21   = 185.21    (§4)
    ...
    # §4: dies at bar 29 = Friday 17:00 UTC, before the next box bar can exist.
...
def test_week_one_short_matches_hand_computed_arithmetic(orders: List) -> None:
    ...
    E_short = box_low - B = 184.00 - 0.21     = 183.79    (§5, symmetric buffer)
...
def test_tp1_is_slightly_less_than_one_r(orders: List) -> None:
    ...
    for o in orders:
        ...
        for k, leg in enumerate(o.exits, start=1):
            assert abs(leg.price - o.entry_price) == pytest.approx(k * height, abs=1e-9)
...
def test_stop_is_exactly_the_opposite_box_edge(
    ...
    for o in orders:
        ...
        if o.direction == 1:
            ...
            assert o.entry_price == pytest.approx(float(bar["High"]) + BUFFER, abs=1e-9)
        else:
