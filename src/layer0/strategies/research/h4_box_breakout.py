...
(stamped **Sunday 21:00 UTC**, spec §3). At that bar's close — Monday 01:00 UTC,
which is therefore the decision bar — the strategy declares two pending stop
orders, one either side of the box, offset by a 21-pip noise buffer (spec §4/§8),
each stopped at the opposite box edge (spec §6) and scaled out in four equal
quarters at 1/2/3/4 box heights (spec §7). Both pendings live 29 H4 bars
(spec §4 step "expires_after_bars"), which kills them at Friday 17:00 UTC —
before the next week's box bar can exist.
...
* **NOTE B — the week-open bar is identified by its stamp, never by position.**
  ...
  re-anchor the box onto a holiday Monday. Spec §3 step 2 and §10 row 8 require the
  literal Sunday-21:00-UTC stamp and require the *whole week to be skipped* when it
  is absent. See the ``_week_open_box_positions`` docstring for the DST caveat this
  creates.
...
(spec §3). ``detect_swing_points`` is irrelevant here — no swing, pivot or fractal is
used (spec §9).
"""
...
def _week_open_box_positions(index: pd.Index) -> np.ndarray:
    ...
    The stamp is matched literally. Consequences, both intended by spec §3 step 2 /
    §10 row 8:
...
class H4BoxBreakout(StrategyV2):
    ...
    # -- spec §4/§8: the only filter in the source is the noise buffer ---------
    ...
    # -- spec §4: 30-bar feed week, box bar = bar 0, order dies after bar 29 ----
    EXPIRY_BARS = 29
    ...
    def metadata(self) -> StrategyMetadataV2:
        return StrategyMetadataV2(
            ...
            granularities=["H4"],
            # Spec §2: the only two pairs the spec lists under "pairs_available" are
            # the Wave-1-addition GBP_JPY and EUR_JPY (both annotated "pending" —
            # backfill in flight, not a fleet exclusion). The spec's explicit
            # instruction is to "declare, harness skips if backfill incomplete", so
    ...
    def _intent(
        ...
        *,
        ...
        NOTE D — every take-profit level is ``declared entry ± k x H``. The stop is
        the opposite box edge exactly (spec §6, §10 row 7): no buffer, no breakeven
        move, no trail. Adding any of those would flatter the results.
        """
        exits = [
            ExitLeg(
