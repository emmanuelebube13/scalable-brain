...
Translates qualified Layer 0 strategies into SQL MERGE scripts that populate
...
def _safe_sql_string(value: str) -> str:
    """Escape single quotes for T-SQL strings."""
...
def _build_merge_strategy_sql(key: str, entry: Dict[str, Any]) -> str:
    """Generate MERGE SQL for Dim_Strategy."""
...
USING (VALUES ('{key}', '{name}', '{desc}', '{stype}', 1))
...
DECLARE @sid_{key} INT = (SELECT Strategy_ID FROM [dbo].[Dim_Strategy] WHERE Strategy_Key = '{key}');
"""


def _build_merge_config_sql(key: str, entry: Dict[str, Any], version: str = "1.0.0") -> str:
    """Generate MERGE SQL for Dim_Strategy_Config."""
...
USING (VALUES (@sid_{key}, '{version}', '{config_hash}', '{granularity}', '{ind_json}', '{rules_json}', {risk_json}, GETUTCDATE(), NULL, 1))
...
DECLARE @cid_{key} INT = (SELECT Config_ID FROM [dbo].[Dim_Strategy_Config]
    WHERE Strategy_ID = @sid_{key} AND Config_Version = '{version}' AND Granularity = '{granularity}');
"""


def _build_merge_mapping_sql(key: str, asset_id: int, priority: int = 100) -> str:
    """Generate MERGE SQL for Dim_Strategy_Asset_Mapping."""
    return f"""
MERGE [dbo].[Dim_Strategy_Asset_Mapping] AS target
USING (VALUES (@sid_{key}, {asset_id}, (SELECT Granularity FROM [dbo].[Dim_Strategy_Config] WHERE Config_ID = @cid_{key}), @cid_{key}, {priority}, GETUTCDATE(), NULL, 1))
...
def generate_sql_seed(
    ...
    """
    Generate a complete T-SQL seed script for Layer 2 tables.
    ...
    Returns:
        T-SQL MERGE script as a string.
    """
    lines = [
        "-- =============================================================================",
    ...
    for result in promotable_results:
        ...
        for asset_symbol in qualified_assets:
            ...
            lines.append(_build_merge_mapping_sql(key, asset_id))
