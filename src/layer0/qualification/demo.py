...
def generate_sample_data(
    n_bars: int = 2000, trend_strength: float = 0.3, volatility: float = 0.008
) -> pd.DataFrame:
    """
    ...
    df["High"] = df[["Open", "Close"]].max(axis=1) * (
        1 + np.abs(np.random.normal(0, daily_range / 2, n_bars))
    )
...
def run_single_strategy_demo():
    ...
    strategy = TrendEMAADXStrategy(fast_ema=20, slow_ema=50, adx_threshold=25.0)

    # Run backtest
    logger.info("\nRunning backtest...")
    engine = BacktestEngine(BacktestConfig(initial_capital=100000))
    result = engine.run_backtest(strategy, df, "EUR_USD", "H4", warmup_bars=200)
...
def run_walk_forward_demo():
    ...
    wf_results = engine.run_walk_forward_analysis(
        strategy, df, "EUR_USD", "H4", train_size=500, test_size=100, n_windows=5
    )
