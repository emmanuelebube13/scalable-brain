...
src_root = next(
    (parent for parent in script_path.parents if parent.name == "src"),
...
from ..core_engine.multi_timeframe import MultiTimeframeEngine, create_mtf_config
from ..data_access import data_loader
from ..promotion import layer2_config_adapter
...
def _param_grid_for_strategy(
    strategy_name: str, granularity: str
) -> List[Dict[str, Any]]:
...
def _evaluate_candidate_walk_forward(
    ...
    diagnostics = {
        ...
        "avg_expectancy_r": (
            float(np.mean([m.expectancy_r for m in valid_windows]))
            if valid_windows
            else 0.0
        ),
        "avg_profit_factor": (
            float(np.mean([m.profit_factor for m in valid_windows]))
            if valid_windows
            else 0.0
        ),
        "avg_drawdown_pct": (
            float(np.mean([m.max_drawdown_pct for m in valid_windows]))
            if valid_windows
            else 0.0
        ),
        "score": score,
...
def load_historical_data(
    ...
    if use_db:
        try:
            ...
            logger.warning(
                f"Failed to load from DB for {asset_symbol} {granularity}: {e}"
            )
    ...
    logger.warning(
        f"No data file found for {asset_symbol} {granularity}, generating synthetic data"
    )
    return generate_synthetic_data(asset_symbol, granularity)


def generate_synthetic_data(
    asset: str, granularity: str, n_bars: int = 5000
) -> pd.DataFrame:
    """
...
def run_strategy_qualification(
        ...
        if symbol in results["asset_results"]:
            for gran in granularities:
                if gran in results["asset_results"][symbol]:
                    if results["asset_results"][symbol][gran]["metrics"].get(
                        "qualified"
                    ):
    ...
    if len(qualified_pairs) >= 1:
        ...
        results["qualification_reason"] = (
            f"Sandbox qualified: {len(qualified_pairs)} asset+granularity pair(s): {', '.join(qualified_pairs)}"
        )
    else:
        results["overall_qualified"] = False
        results["qualification_reason"] = (
            f"No asset+granularity pairs met qualification criteria"
        )
    ...
    if all_metrics:
        ...
        if valid_metrics:
            results["aggregate"] = {
                ...
                "avg_max_drawdown_pct": np.mean(
                    [m.max_drawdown_pct for m in valid_metrics]
                ),
...
def preload_historical_data(
    ...
    for symbol in asset_symbols:
        ...
        if asset_id is None:
            logger.warning(
                f"Asset symbol '{symbol}' not found in Dim_Asset mapping; skipping"
            )
            continue
...
def generate_qualification_report(
    all_results: List[Dict], output_dir: str = "./results"
) -> str:
    """
    ...
    with open(md_path, "w") as f:
        ...
        if qualified:
            f.write(
                "| Strategy | Assets Qualified | Avg Win Rate | Avg Expectancy (R) |\n"
            )
            f.write(
                "|----------|------------------|--------------|-------------------|\n"
            )
            for r in qualified:
                ...
                f.write(
                    f"{agg.get('avg_win_rate', 0):.2%} | {agg.get('avg_expectancy_r', 0):.3f} |\n"
                )
        else:
        ...
        for r in qualified:
            ...
            f.write(
                "| Asset | Granularity | Trades | Win Rate | Expectancy (R) | Profit Factor | Max DD | Qualified |\n"
            )
            f.write(
                "|-------|-------------|--------|----------|----------------|---------------|--------|-----------|\n"
            )
            ...
            for asset, asset_results in r["asset_results"].items():
                for gran, result in asset_results.items():
                    ...
                    f.write(
                        f"{m['profit_factor']:.3f} | {m['max_drawdown_pct']:.2%} | "
                    )
                    f.write(f"{'Yes' if m['qualified'] else 'No'} |\n")
                    if opt.get("optimized") and opt.get("best_params"):
                        f.write(
                            f"  - Optimized Params ({asset} {gran}): {opt.get('best_params')}\n"
                        )
            f.write("\n")
...
def _get_strategy_regime_type(strategy_name: str) -> str:
    ...
    if any(
        ...
        for keyword in ["range", "bollinger", "stochastic", "mean_reversion"]
    ):
        return "RANGING"

    # Breakout strategies
    if any(
        keyword in name_lower for keyword in ["breakout", "vcp", "support_resistance"]
    ):
        return "BREAKOUT"
...
def generate_all_strategy_mappings(
    ...
    for strategy in strategies:
        ...
        logger.info(
            f"  {strategy_name}: mapped to {len(asset_symbols)} assets (regime: {regime_type})"
        )
...
def generate_bypass_sql_seed(
    ...
    strategies_by_regime: Dict[str, List[str]] = {
        "TRENDING": [],
        "RANGING": [],
        "BREAKOUT": [],
        "UNKNOWN": [],
    ...
    for result in all_results:
        ...
        if entry is None:
            lines.append(
                f"-- WARNING: No Layer 2 catalog mapping for strategy '{key}'. Skipped."
            )
            continue
        ...
        name = key
        desc = layer2_config_adapter._safe_sql_string(
            f"{entry['description']} [REGIME:{regime_type}]"
        )
        stype = entry["strategy_type"]

        lines.append(f"-- Strategy: {key} (Regime: {regime_type})")
        lines.append(
            f"""INSERT INTO Dim_Strategy (Strategy_Key, Strategy_Name, \"Description\", Strategy_Type, Is_Active)
...
ON CONFLICT (Strategy_Key) DO UPDATE SET
    ...
    Modified_Date = NOW();"""
        ...
        risk_json = (
            "NULL"
            if risk is None
            else f"'{layer2_config_adapter._safe_sql_string(json.dumps(risk))}'"
        )
        config_hash = layer2_config_adapter._compute_config_hash(
            indicators, rules, risk
        )

        lines.append(
            f"""INSERT INTO Dim_Strategy_Config (Strategy_ID, Config_Version, Config_Hash, Granularity, Indicator_Configs, Signal_Rules, Risk_Filters, Effective_From, Effective_To, Is_Active)
VALUES (
...
ON CONFLICT (Strategy_ID, Config_Version, Granularity) DO UPDATE SET
    ...
    Is_Active = EXCLUDED.Is_Active;"""
        ...
        for asset_symbol in all_assets:
            ...
            lines.append(
                f"""WITH sid AS (SELECT Strategy_ID FROM Dim_Strategy WHERE Strategy_Key = '{key}'),
...
def main():
    ...
    parser.add_argument(
        "--granularities", nargs="+", default=["H4", "H1"], help="Timeframes to test"
    )
    ...
    if use_db:
        try:
            ...
            logger.error(
                "Use --no-use-db to run with CSV/synthetic data only, or check DB credentials."
            )
            sys.exit(1)
    else:
        ...
        logger.warning(
            "Running in offline mode with hardcoded asset map (symbol -> id)"
        )
    ...
    disqualified_results = [
        r
        for r in all_results
        if not r.get("overall_qualified") and r.get("status") != "failed"
    ]
    logger.info(f"Total strategies tested: {len(strategies)}")
    logger.info(f"Strategies qualified (sandbox): {len(qualified_results)}")
    logger.info(
        f"Strategies disqualified (0 qualified pairs): {len(disqualified_results)}"
    )
    ...
    if qualified_results:
        ...
        for r in qualified_results:
            logger.info(
                f"  - {r['strategy_name']}: {len(r['qualified_assets'])} assets"
            )
