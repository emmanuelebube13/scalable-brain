...
_TRADE_COLUMNS = [
    "timestamp", "asset_id", "strategy_id", "granularity", "trade_horizon", 
    "is_winner", "r_multiple", "holding_bars", "atr_sl_multiplier", "atr_tp_multiplier",
    "entry_signal_type", "exit_reason", "is_oos", "fold_id", "leg_index", "is_terminal_leg"
]
...
def run(lookback_years: int = 10, dry_run: bool = False, only_strat: str = None):
    ...
    logger.info("Preloading prices: %s x %s (%dy)...", symbols, granularities, lookback_years)
    ...
    for record in strats:
        ...
        if record.engine == "backtest_engine_v1":
            gran = record.primary_granularity or getattr(obj.config, "primary_granularity", "H1")
            for symbol in symbols:
                if symbol not in data or gran not in data[symbol]: continue
                import copy
                run_strat = copy.deepcopy(obj)
                res = v1_engine.run_backtest(run_strat, data[symbol][gran], symbol, gran, run_strat.get_required_warmup_bars())
                for t in res.trades:
                    if t.exit_time is None: continue
                    ts = t.entry_time
                    if getattr(ts, "tzinfo", None) is None: ts = ts.replace(tzinfo=timezone.utc)
                    collected.append((
                        ts, asset_map[symbol], sid, gran, gran,
                        1 if (t.pnl or 0.0) > 0 else 0,
                        float(t.r_multiple) if t.r_multiple is not None else None,
                        int(t.bars_held or 0), None, None,
            ...
            for symbol in meta.pairs:
                ...
                for cg in meta.context_granularities:
                    if cg not in data[symbol]: skip = True; break
                ...
                try:
                    ...
                    logger.warning("Skipping %s on %s: %s", record.strategy_key, symbol, e)
                    continue
                    
                if not intents: continue
                
                res = v2_engine.run(frames[gran], intents, pair=symbol, warmup_bars=obj.warmup_bars, strategy=obj, granularity=gran)
                
                for _, t in res.trades.iterrows():
                    if pd.isna(t["exit_time"]): continue
                    ts = t["entry_time"]
                    if getattr(ts, "tzinfo", None) is None: ts = ts.replace(tzinfo=timezone.utc)
                    collected.append((
                        ts, asset_map[symbol], sid, gran, gran,
