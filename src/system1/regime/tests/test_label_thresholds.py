...
direction feature and the ``volatility_20 + atr_14`` sum — so each state is materialised
as a row carrying exactly those, with the vol sum placed in ``atr_14``.
"""
...
FN: List[str] = ["atr_14", "adx_14", "volatility_20", "returns_1", "trend_20"]
DIRECTION = "trend_20"
...
def _means(gran: str) -> np.ndarray:
    ...
    for _state, trend, vol_sum, _share, _old in sorted(DOC_STATES[gran]):
        # vol+atr is what the rule uses; carry the whole sum in atr_14, vol at 0.0.
...
def test_high_vol_assignment_is_unchanged(gran: str) -> None:
    """High-Vol is still the single argmax of volatility_20 + atr_14 — the fix does not
    touch that step, nor the deferred question of it absorbing downtrends (§5)."""
    means = _means(gran)
    expected = int(
        np.argmax(means[:, FN.index("volatility_20")] + means[:, FN.index("atr_14")])
    )
