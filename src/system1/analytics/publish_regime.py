...
def build_document() -> Dict[str, Any]:
    ...
    # Load the latest regime_causal per asset and granularity
    # We need d1_trend for all strategies. d1_trend means model_id = 'trend_d1'
    # Wait! In R2 we used `resolve_regime_labels` which queries:
    # "SELECT bar_timestamp, regime_causal FROM fact_market_regime_v2 WHERE asset_id = %s AND granularity = %s AND model_id = %s ORDER BY bar_timestamp"
    # So model_id is 'd1_trend' (or is it 'trend_d1'?). Let's check context.py or families.py.
    # The requirement says: "regime_source: 'd1_trend' or 'hmm_causal'"
    # In R2 we called: resolve_regime_labels(conn, pair, granularity, ..., "d1_trend")
    # Actually, R4 says "Only regime_causal".
    
    from src.layer0.strategies.research_data import load_ohlcv_readonly
    from src.regime_aware.context import build_trend_labels
    
    # Pre-calculate d1_trend regimes for all pairs
    pair_d1_regime = {}
    for asset_id, pair in asset_symbols.items():
        d1_df = load_ohlcv_readonly(pair, "D1", lookback_years=1)
...
def publish(payload: Dict[str, Any], payload_path: str, storage=None) -> Dict[str, Any]:
    ...
    try:
        storage.put_object(f"{remote_prefix}/regime_status.json", payload_path, encrypt=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Publish the S1 regime status document")
    parser.add_argument("--dry-run", action="store_true", help="Build + stage locally; do not upload.")
