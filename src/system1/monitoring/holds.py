...
def parse_holds(payload: dict[str, Any], now: datetime) -> tuple[dict[str, Hold], list[str]]:
    problems = []
    holds_by_check = {}
    ...
    for i, h in enumerate(payload["holds"]):
        ...
        expected_keys = {"checks", "reason", "declared_by", "declared_at_utc", "expires_utc", "evidence"}
        ...
        try:
            declared_at = datetime.fromisoformat(h["declared_at_utc"].replace("Z", "+00:00"))
...
def summarise(holds_by_check: dict[str, Hold], problems: list[str], now: datetime) -> CheckResult:
    if problems:
    ...
    return CheckResult(
        "holds", Status.OK,
        f"{count} {plural} active, soonest expires in {soonest}d"
    )
