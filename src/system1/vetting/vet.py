...
def _load_cells() -> tuple[List[Dict], str]:
    ...
    with engine.connect() as conn:
        run_id = conn.execute(
            text("SELECT qualification_run_id FROM fact_strategy_regime_attribution "
...
def build(cells: List[Dict], run_id: str) -> Dict[str, Any]:
    rejection = {k: 0 for k in ["pf_fail", "sharpe_fail", "maxdd_fail", "winrate_fail",
                                "recovery_fail", "oos_fail", "low_confidence_fail"]}
    ...
    for c in cells:
        ...
        if passed:
            ...
            rejection_detail.append({
                "strategy_id": c["strategy_id"], "variant": c["variant"],
                "regime": c["regime"], "failed_gates": failures,
            })
    ...
    for regime in REGIMES:
        ...
        if not ranked:
            ...
            logger.warning("STARVATION: no qualifying strategies for regime '%s'", regime)
            continue
        regimes_out[regime] = [
            {"strategy_id": c["strategy_id"], "variant": c["variant"], "rank": c["rank"],
             "composite_score": round(c["composite_score"], 6), "metrics": _metrics_block(c)}
            for c in ranked
        ]
        weights_out[regime] = {k: round(v, 8) for k, v in G.normalized_weights(ranked).items()}

    now = datetime.now(timezone.utc).isoformat()
    regime_map = {
        "schema_version": SCHEMA_VERSION, "generated_at_utc": now,
        "regime_model_version": REGIME_MODEL_VERSION, "qualification_run_id": run_id,
        "ranking_rule": G.RANKING_RULE, "gates": G.GATES,
        "regimes": regimes_out, "empty_regimes": empty_regimes,
...
def _update_registry(regime_map: Dict) -> None:
    ...
    with engine.begin() as conn:
        conn.execute(text("ALTER TABLE dim_strategy_registry ADD COLUMN IF NOT EXISTS is_qualified boolean"))
        conn.execute(text("UPDATE dim_strategy_registry SET is_qualified = false"))
        if qualified_ids:
            conn.execute(
                text("UPDATE dim_strategy_registry SET is_qualified = true WHERE strategy_id = ANY(:ids)"),
...
def run(live: bool = False, register_mlflow: bool = True) -> Dict[str, Any]:
    ...
    if live:
        ...
        _write_json(os.path.join(REPORTS_DIR, "proposed_regime_strategy_map.json"), out["map"])
        _write_json(os.path.join(REPORTS_DIR, "proposed_strategy_weights.json"), out["weights"])
        map_path = os.path.join(REPORTS_DIR, "proposed_regime_strategy_map.json")

    report = {
        "qualification_run_id": run_id, "generated_utc": datetime.now(timezone.utc).isoformat(),
        "mode": "live" if live else "log_only", "n_cells": len(cells),
    ...
    summary = {k: report[k] for k in ("n_cells", "n_qualifying", "empty_regimes", "mode")}
...
def main() -> None:
    ...
    p.add_argument("--live", action="store_true", help="write to results/state/ and update registry")
