...
def build_query_with_contract(engine_obj):
    ...
    if 'Indicator_Snapshot' in fs_cols:
        ...
        optional_cols.append("fs.Indicator_Snapshot")
...
def engineer_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'H4_Trend_Direction' in df.columns and 'D1_Trend_Direction' in df.columns:
        ...
        df['H4_D1_Agreement'] = (df['H4_Trend_Direction'] == df['D1_Trend_Direction']).astype(int)
    ...
    if 'Candle_Body' in df.columns and 'Upper_Wick' in df.columns:
        # Candle sentiment
        df['Candle_Sentiment'] = df['Candle_Body'] / (df['Upper_Wick'] + df['Lower_Wick'] + 1e-8)
...
def calculate_strategy_performance_features(
    ...
    for strategy_id in df['Strategy_ID'].unique():
        ...
        for window in lookback_windows:
            # Rolling win rate
            rolling_wins = strat_df['Is_Winner'].rolling(window=window, min_periods=5).mean()
            ...
            if 'R_Multiple' in strat_df.columns:
                rolling_exp = strat_df['R_Multiple'].rolling(window=window, min_periods=5).mean()
                df.loc[mask, f'Strat_Expectancy_{window}'] = rolling_exp.values
    
    # Strategy recency (how recently has this strategy traded)
    df['Bars_Since_Last_Trade'] = df.groupby('Strategy_ID')['Timestamp'].diff().dt.total_seconds() / 3600  # hours
...
def comprehensive_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    print(f"[Feature Engineering] Starting with {len(df)} rows, {len(df.columns)} columns")
    
    # Step 1: Extract indicator snapshot features
    df = extract_indicator_snapshot_features(df)
    print(f"[Feature Engineering] After indicator extraction: {len(df.columns)} columns")
    ...
    print(f"[Feature Engineering] After strategy performance: {len(df.columns)} columns")
    ...
    for col in categorical_cols:
        if col not in ['Timestamp', 'Indicator_Snapshot', 'Signal_Reason', 'Config_Hash', 'Batch_ID']:
            df[col] = df[col].astype('category')
    
    # Report feature statistics
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    print(f"[Feature Engineering] Final: {len(df)} rows, {len(df.columns)} columns ({len(numeric_cols)} numeric)")
...
def optuna_objective(trial, model_type, X_train_df, y_train_series):
    ...
    for train_idx, val_idx in tscv.split(X_train_df):
        ...
        calibration_bonus = min(0.05, prob_std * 0.1)  # Small bonus for spread-out probabilities
...
def choose_threshold(
    ...
    if best_t is None and len(prob) > 0:
        ...
        if target_count > 0 and target_count <= len(sorted_probs):
            ...
            if metrics['expectancy_unit_r'] > min_expectancy * 2:  # Allow 2x the negative slack
                best_t = adaptive_threshold
                print(f"[Threshold] Using adaptive percentile-based threshold: {best_t:.4f} "
...
def print_threshold_diagnostics(
    ...
    if len(feasible_rows) > 0:
        ...
        print(feasible_rows.head(top_n)[['threshold', 'turnover', 'expectancy_unit_r', 'precision_at_selected', 'pr_auc', 'f1', 'meets_gates']].to_string(index=False))
    else:
        ...
        df_diag['turnover_dist'] = df_diag.apply(
            lambda r: 0 if min_turnover <= r['turnover'] <= max_turnover 
            else min(abs(r['turnover'] - min_turnover), abs(r['turnover'] - max_turnover)),
            axis=1
        )
        closest = df_diag.nsmallest(top_n, 'turnover_dist')
        print(closest[['threshold', 'turnover', 'expectancy_unit_r', 'precision_at_selected', 'pr_auc', 'f1']].to_string(index=False))
...
def train_models(
    ...
    for model_type in model_types:
        ...
        try:
            ...
            best_model = tree_model_factory(model_type, best_params, class_ratio, calibrate=True)
            best_model.fit(X_train_t, y_train)
            
            # Check probability distribution
            prob_check = best_model.predict_proba(X_train_t)[:, 1]
            print(f"  {model_type} probability stats: mean={prob_check.mean():.4f}, std={prob_check.std():.4f}, "
            ...
            if threshold is None:
                print(f"[WARN] {model_type}: no threshold satisfies strict gates, trying fallback...")
            ...
            if not passes_gates:
                print(f"[WARN] {model_type} does not pass test gates (turnover={metrics['turnover']:.4f}, "
    ...
    try:
        ...
        if threshold is None:
            print(f"[WARN] LSTM: no threshold satisfies strict gates, trying fallback...")
        ...
        if passes_gates:
            ...
            print(f"[WARN] LSTM does not pass test gates (turnover={metrics['turnover']:.4f}, "
                  f"expectancy={metrics['expectancy_unit_r']:.4f}), but keeping as candidate")
    except Exception as e:
...
def main():
    ...
    parser.add_argument('--dry-run-top-n', type=int, default=2000,
    ...
    meta_cols = ['Timestamp', 'Granularity_Key', 'Indicator_Snapshot', 'Config_Hash', 'Batch_ID']
    ...
    if args.dry_run_load:
        ...
        class_ratio = dynamic_pos_weight(y_train)
        print(f"Dry-run load check passed | rows={len(df)} | features={len(feature_cols)} | class_ratio={class_ratio:.4f}")
