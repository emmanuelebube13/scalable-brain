...
def safe_comprehensive_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.debug(
        f"Feature engineering start: {len(df)} rows, {len(df.columns)} columns"
    )
...
def align_features_for_inference(
    df: pd.DataFrame, expected_columns: List[str], fill_value: float = 0.0
) -> pd.DataFrame:
    """
    ...
    """
    logger.debug(
        f"Aligning features: {len(df.columns)} input -> {len(expected_columns)} expected"
    )
...
def ensure_feature_columns_exist(
...
def validate_inference_data(
    df: pd.DataFrame, expected_columns: List[str], raise_on_error: bool = False
    ...
    if missing and raise_on_error:
        raise ValueError(f"Missing {len(missing)} required columns: {sorted(missing)}")
