...
class SignalEngine:
    ...
    def run(
        ...
        try:
            ...
            for granularity in granularities:
                ...
                logger.debug(
                    f"Strategy details: {[(s.strategy_id, s.asset_id, s.strategy_name) for s in strategies]}"
                )
                ...
                # Process each asset
                logger.debug(
                    f"Starting asset processing loop with {len(strategies_by_asset)} assets"
                )
    ...
    def _load_strategies(
        ...
        """
        # NOTE (FND-004 Phase 3): columns are aliased back to the mixed-case
        # names StrategyConfig.from_db_row expects. The live schema currently
        # lacks dim_strategy.strategy_key and dim_strategy_asset_mapping.priority
        # (and the *_configs columns are jsonb, so from_db_row's json.loads may
        # need adjusting). The strategy tables are empty, so this path could not
        # be validated end-to-end — see CODE_MIGRATION_PHASE3.md.
        query = """
            SELECT
                s.strategy_id        AS "Strategy_ID",
                s.strategy_name      AS "Strategy_Key",
                s.strategy_name      AS "Strategy_Name",
                c.config_id          AS "Config_ID",
                c.config_version     AS "Config_Version",
                c.config_hash        AS "Config_Hash",
                c.granularity        AS "Granularity",
                c.indicator_configs  AS "Indicator_Configs",
                c.signal_rules       AS "Signal_Rules",
                c.risk_filters       AS "Risk_Filters",
        ...
        if strategy_ids:
            placeholders = ",".join(["%s" for _ in strategy_ids])
            query += f" AND s.strategy_id IN ({placeholders})"
            params.extend(strategy_ids)

        query += " ORDER BY s.strategy_id"
    ...
    def _process_strategy(
        self, df: pd.DataFrame, asset_id: int, strategy_config: StrategyConfig
    ) -> SignalResult:
        """
