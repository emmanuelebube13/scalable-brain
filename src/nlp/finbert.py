...
class ScalableBrainFinBERT:
    ...
    def get_features(text: str, auto_chunk: bool = True) -> Dict:
        ...
        if auto_chunk and len(tokens) > 512:
            ...
            chunk_features = [
                ScalableBrainFinBERT._process_single_chunk(c) for c in chunks
            ]
    ...
    def batch_features(text_list: List[str]) -> List[Dict]:
        ...
        for i, text in enumerate(text_list):
            ...
            if len(tokens) < 3:
                ...
                continue
            chunks = (
                ScalableBrainFinBERT._chunk_text(text) if len(tokens) > 512 else [text]
            )
        ...
        for start in range(0, len(chunk_list), ScalableBrainFinBERT.BATCH_SIZE):
            ...
            with torch.no_grad():
                outputs = model(**inputs)
                probs_batch = torch.nn.functional.softmax(
