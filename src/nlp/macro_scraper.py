...
class FactMacroEvents(Base):
    """ORM model mapped to the live ``fact_macro_events`` table.

    FND-004 Phase 3: the live schema is narrower than the original design, so
    derived features (surprise score, FinBERT dispersion, cyclical time
    encodings, asset_id) are not persisted as dedicated columns — the surprise
    and dispersion are summarised into ``description`` instead.
    """
...
class MacroIngestionPipeline:
    ...
    def __init__(self, db_connection_string: Optional[str] = None):
        # Connectivity routes through the canonical PostgreSQL engine
        # (src.common.db); the argument is retained for backwards compatibility
        # but ignored (was an mssql+pyodbc URL).
    ...
    def _compute_surprise_score(
        actual: float, consensus: float, previous: float
    ) -> float:
    ...
    async def _fetch_forex_factory_xml(
                ...
                events.append(
                    {
                        ...
                        "event_title": title,
                        "actual": (
                            float(actual_str.replace("%", "").replace(",", ""))
                            if actual_str and actual_str != "-"
                            else None
                        ),
                        "forecast": (
                            float(forecast_str.replace("%", "").replace(",", ""))
                            if forecast_str and forecast_str != "-"
                            else None
                        ),
                        "previous": (
                            float(previous_str.replace("%", "").replace(",", ""))
                            if previous_str and previous_str != "-"
                            else None
                        ),
                        "source": "ForexFactory",
                    }
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            ecb_texts = await self._fetch_rss(
                session, "https://www.ecb.europa.eu/rss/press.xml"
            )
            fed_texts = await self._fetch_rss(
                session, "https://www.federalreserve.gov/feeds/fed_speeches.xml"
            )
            all_news_texts = ecb_texts + fed_texts

            # 2. FINBERT – ONE BATCH FOR ALL NEWS
            news_features = (
                self.finbert.batch_features(all_news_texts) if all_news_texts else []
            )
            ...
            calendar_finbert = (
                self.finbert.batch_features(calendar_titles) if calendar_titles else []
            )
            ...
            with self.Session() as session_db:
                try:
                    ...
                    for i, feat in enumerate(news_features):
                        records.append(
                            FactMacroEvents(
                                timestamp=now_utc,
                                source="ECB/Fed",
                                title=all_news_texts[i][:200],
                                description=(
                                    f"surprise=0.0; dispersion={feat['dispersion']}"
                                ),
                    ...
                    for i, ev in enumerate(calendar_events):
                        surprise = self._compute_surprise_score(
                            ev["actual"], ev["forecast"], ev["previous"]
                        )
                        feat = (
                            calendar_finbert[i]
                            if i < len(calendar_finbert)
                            else {"sentiment_score": 0.0, "dispersion": 0.0}
                        )
                        records.append(
                            FactMacroEvents(
                                ...
                                description=(
                                    f"surprise={surprise}; dispersion={feat['dispersion']}"
                                ),
                                sentiment_score=feat["sentiment_score"],
                                created_at=now_utc,
                            )
                    ...
                    if records:
                        ...
                        session_db.commit()
                        logger.info(
                            f"✅ Inserted {len(records)} macro events into fact_macro_events"
                        )
...
if __name__ == "__main__":
    ...
    # Connectivity is built from .env by src.common.db (PostgreSQL).
    pipeline = MacroIngestionPipeline()
