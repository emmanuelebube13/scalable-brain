...
def test_unrecognised_selection_basis_refused(caplog):
    ...
    bars_df = pd.DataFrame([{"instrument": "EUR_USD", "timestamp": datetime.now(timezone.utc), "Close": 1.05}])
    current_regimes = {"EUR_USD": "Trending-Up"}
...
def test_designated_strategy_carries_basis():
    ...
    bars_df = pd.DataFrame([{"instrument": "EUR_USD", "timestamp": datetime.now(timezone.utc), "Close": 1.05, "granularity": "H1"}])
    current_regimes = {"EUR_USD": "Trending-Up"}
