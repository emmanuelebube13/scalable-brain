...
def get_live_trades(
    ...
    for i, r in enumerate(rows):
        ...
        trade: Dict[str, Any] = {
            ...
            "pnl": 1.0 if outcome == "win" else (-1.0 if outcome == "loss" else None),
            "slippage": None,
            "holdDuration": hold_duration,
            "outcome": outcome if is_approved else None,
            "vetoReason": f"ML confidence below {os.getenv('LAYER3_APPROVAL_THRESHOLD', '0.20')} threshold" if not is_approved else None,
        }
...
def get_kpi_data(engine: sa.engine.Engine) -> Dict[str, Any]:
    ...
    # Use 24h window for signals but verify it's reasonable
    query_str = """
        SELECT
            COUNT(*) AS total_signals,
            SUM(CAST(flt.Is_Approved AS INTEGER)) AS approved_count,
...
def get_open_positions(
    ...
    query = sa.text(f"""
        ...
        GROUP BY da.Symbol,
                 CASE WHEN flt.Signal_Value >= 0 THEN 'long' ELSE 'short' END,
                 flt.Entry_Price,
                 flt.Order_ID,
                 {event_ts_expr}
        ORDER BY {event_ts_expr} DESC
