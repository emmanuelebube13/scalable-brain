...
#: Every regime enabled. The blind arm runs under this so that it is filtered by
#: UNKNOWN exactly as the aware arm is, leaving the mask as the only difference
#: between them. Also the mask given to `unclassified` strategies, which therefore
#: reproduce their own blind arm and act as a null control: any aware-vs-blind
#: difference there is measurement noise, not the intervention.
#: Reused rather than redefined so there is exactly one all-permissive mask in the
#: codebase and the blind arm cannot drift from the `unclassified` control.
PERMISSIVE_MASK = REGIME_MASKS["unclassified"]
...
def main():
    ...
    # Every discovered strategy runs. `unclassified` is not an exclusion — it gets the
    # all-permissive mask, so its aware arm reproduces its blind arm and it serves as a
    # null control. Dropping those strategies would also make the coverage count silently
    # disagree with the discovered fleet.
    v2_classified = {}
    ...
    n_unclassified = sum(1 for _, f in v2_classified.values() if f == "unclassified")
    logger.info(
        f"Running {len(v2_classified)} v2 strategies "
        f"({n_unclassified} unclassified, running as null controls)..."
    )
    ...
    for sid, (strategy, family) in sorted(v2_classified.items()):
        ...
        for pair in strategy.metadata.pairs:
            ...
            struct_labels = resolve_regime_labels(conn, pair, granularity, frames[granularity].index, "structural")

            # --- BLIND ARM: gated with an ALL-PERMISSIVE mask, once per label source.
            blind_d1_df = _run_engine(
                engine, RegimeGateV2(strategy, d1_labels, PERMISSIVE_MASK),
                frames, pair, granularity)
            blind_hmm_df = _run_engine(
                engine, RegimeGateV2(strategy, hmm_labels, PERMISSIVE_MASK),
                frames, pair, granularity)
            blind_struct_df = _run_engine(
                engine, RegimeGateV2(strategy, struct_labels, PERMISSIVE_MASK),
                frames, pair, granularity)
            
            blind_trades_df = blind_d1_df
            ...
            def prep_trades(tdf, arm_name, r_source, mask_dict, labels_series):
                ...
                for _, t in tdf.iterrows():
                    # The gate runs on decision_bar, so the audit log must fetch the regime at decision_bar
                    decision_dt = pd.Timestamp(t["decision_time"])
                    if decision_dt.tzinfo is None:
                        decision_dt = decision_dt.tz_localize("UTC")
                        
                    regime_val = str(labels_series.loc[decision_dt]) if decision_dt in labels_series.index else str(UNKNOWN)
                    
                    # We still record entry_time in the timestamp field for general continuity
            ...
            # Each label source gets its own matched blind baseline.
            db_rows = []
            db_rows.extend(prep_trades(blind_d1_df, "blind", "d1_trend", PERMISSIVE_MASK, d1_labels))
            db_rows.extend(prep_trades(blind_hmm_df, "blind", "hmm_causal", PERMISSIVE_MASK, hmm_labels))
            db_rows.extend(prep_trades(blind_struct_df, "blind", "structural", PERMISSIVE_MASK, struct_labels))
            ...
            if a_hmm_count > 0:
                ...
                db_rows.extend(prep_trades(aware_hmm_trades_df, "aware", "hmm_causal", mask, hmm_labels))

            # --- AWARE ARM (structural) ---
            gate_struct = RegimeGateV2(strategy, struct_labels, mask)
            aware_struct_trades_df = _run_engine(engine, gate_struct, frames, pair, granularity)
            a_struct_count = len(aware_struct_trades_df)
            if a_struct_count > 0:
                aware_r_struct.extend(aware_struct_trades_df["r_multiple"].tolist())
                db_rows.extend(prep_trades(aware_struct_trades_df, "aware", "structural", mask, struct_labels))
        ...
        struct_df = pd.DataFrame({"r_multiple": aware_r_struct})
        
        comp_d1 = compare_arms(blind_df, d1_df)
        comp_hmm = compare_arms(blind_df, hmm_df)
        comp_struct = compare_arms(blind_df, struct_df)
        ...
        comparisons.append({
            ...
            "struct_trades": len(aware_r_struct),
            "struct_uplift": comp_struct["mean_uplift_r"],
            "struct_p": comp_struct["p_value"]
        })
        
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)
        with open(RESULTS_DIR / "STATE_APPends.md", "a") as f:
            f.write(f"| {datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%MZ')} | R3 | step 4 | DONE | {sid}: blind={len(blind_r)}, d1={len(aware_r_d1)}, hmm={len(aware_r_hmm)}, struct={len(aware_r_struct)} |\n")
            
        logger.info(f"[{sid}] Completed. Blind: {len(blind_r)}, D1: {len(aware_r_d1)}, HMM: {len(aware_r_hmm)}, Struct: {len(aware_r_struct)}")
    ...
    with open(RESULTS_DIR / "R3_COMPARISON_REPORT.md", "w") as f:
        ...
        f.write(f"Total comparisons made: {len(comparisons) * 3}\n")
        f.write(f"Starvation cells (trades < 30): {starvation_cells}\n")
        f.write(f"Zero-trade cells: {zero_trade_cells}\n\n")
        f.write("| Strategy | Blind Trades | D1 Trades | D1 Uplift (R) | HMM Trades | HMM Uplift (R) | Struct Trades | Struct Uplift (R) |\n")
        f.write("|---|---|---|---|---|---|---|---|\n")
        for c in comparisons:
            f.write(f"| {c['strategy']} | {c['blind_trades']} | {c['d1_trades']} | {c['d1_uplift']:.4f} | {c['hmm_trades']} | {c['hmm_uplift']:.4f} | {c['struct_trades']} | {c['struct_uplift']:.4f} |\n")
