...
def ensure_strategy_id(
    ...
    *,
    ...
    if universe not in VALID_UNIVERSES:
        raise ValueError(f"Unknown universe '{universe}'. Valid choices: {VALID_UNIVERSES}")
    ...
    with db_engine.begin() as conn:
        ...
        """)
        conn.execute(ins_reg_sql, {
            "id": next_id,
            "name": strategy_key
        })
        
        logger.info(f"Allocated strategy_id {next_id} for strategy_key '{strategy_key}'")
        return int(next_id)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Allocate strategy IDs for all universes")
    parser.add_argument("--dry-run", action="store_true", help="Print what would be allocated without saving")
    ...
    def process_strat(key, universe, engine, gran, family):
        ...
        if args.dry_run:
            ...
            sid = ensure_strategy_id(key, universe=universe, engine=engine, primary_granularity=gran, family=family)
            print(f"{key:<40} | {sid:<4} | {universe:<20} | {engine:<20}")
        count += 1
            
    for s in legacy_strats:
        process_strat(s.config.name, "legacy", "backtest_engine_v1", getattr(s.config, "granularity", "H1"), None)
