...
def all_strategies() -> List[StrategyRecord]:
    ...
    with db_engine.connect() as conn:
        result = conn.execute(text("SELECT strategy_id, strategy_key, universe, engine, primary_granularity, family FROM dim_strategy ORDER BY strategy_id")).mappings().all()
...
def by_id(strategy_id: int) -> StrategyRecord:
    ...
    with db_engine.connect() as conn:
        row = conn.execute(
            text("SELECT strategy_id, strategy_key, universe, engine, primary_granularity, family FROM dim_strategy WHERE strategy_id = :id"),
            {"id": strategy_id}
        ).mappings().fetchone()
...
def by_key(strategy_key: str) -> StrategyRecord:
    ...
    with db_engine.connect() as conn:
        row = conn.execute(
            text("SELECT strategy_id, strategy_key, universe, engine, primary_granularity, family FROM dim_strategy WHERE strategy_key = :key"),
            {"key": strategy_key}
        ).mappings().fetchone()
    ...
    if record.universe == "legacy":
        ...
        for s in strats:
            if getattr(s, "config", None) and s.config.name == record.strategy_key:
                return s
        raise ValueError(f"Legacy strategy {record.strategy_key} not found in get_all_strategies()")
        ...
        if record.strategy_key not in classes:
            raise ValueError(f"v2 strategy {record.strategy_key} not found in discover()")
        ...
        for _, name, _ in pkgutil.iter_modules(src.regime_aware.strategies.__path__):
            ...
            if hasattr(mod, "build_regime_aware"):
                ...
                if getattr(strat, "config", None) and strat.config.name == record.strategy_key:
                    return strat
