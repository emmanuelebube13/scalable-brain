...
| FIX-S1-002 | "OOS≥60mo" gate measures in-sample span, not true out-of-sample | System 1 | **P1** | ✅ Verified (log-only) — true OOS via walk-forward folds; `oos_fail` 0→8, 11/80 cells now below the 60-mo gate; gate-can-fire regressions (overfit fails OOS, low-oos rejected) green; independently re-verified (39 tests green, pure walk-forward calendar math reviewed); live map untouched, pending promotion sign-off | — | [link](system-1/FIX-S1-002-oos-not-true-out-of-sample.md) |
| FIX-S1-003 | Regimes don't discriminate strategy performance (map premise inert) | System 1 | **P1** | ✅ Verified (documented finding) — measured on **causal** labels (FIX-S1-005): 0/10 strategies discriminate under both entry-only AND dominant-over-trade-life tagging (hypothesis #1 rejected); regime dimension confirmed cosmetic; recommend NOT building signal-gen regime-filtering, treat map as v1 single-strategy. Live map untouched | — | [link](system-1/FIX-S1-003-regimes-do-not-discriminate.md) |
...
| FIX-S1-005 | Regime labels non-causal (in-sample HMM fit + smoothing) leak future into attribution + gatekeeper OOS | System 1 | **P1** | ✅ Verified (log-only) — causal walk-forward labels emitted+consumed; leakage regression + guard-can-fire counter-test (smoothed DOES leak) green; OOS uplift did NOT shrink (0.0319→0.0405, still sig); independently re-verified (18 tests green, filtered forward-only posterior core reviewed); live champion/map untouched, pending sign-off | — | [link](system-1/FIX-S1-005-regime-labels-non-causal-leakage.md) |
| FIX-S1-006 | Retrain deployment gates `oos_uplift_ok` & `beats_incumbent` never reject | System 1 | **P1** | ✅ Verified (log-only) — both inert gates armed: `oos_uplift_ok` now requires `uplift≥MIN_UPLIFT and significant` (MODEL-006 threaded in, fails CLOSED on missing result / `--allow-missing-uplift` override), serializer persists `regime_accuracy` so `beats_incumbent` can compare; gate-can-fire tests red-before/green-after; 105→114 passed; live champion/map untouched, pending sign-off | — | [link](system-1/FIX-S1-006-deployment-gates-never-reject.md) |
| FIX-S1-007 | `_incumbent()` reads a local pointer while promotion publishes to the storage backend, so `beats_incumbent` never binds on GCS | System 1 | **P1** | ✅ Verified (fix landed) — surfaced during the 2026-07-01 live promotion (GCS `latest` advanced, local `latest.json` stale); `_incumbent()` now reads the pointer + metadata through `build_storage()`; regression test red-before/green-after; full suite 125 passed | — | [link](system-1/FIX-S1-007-incumbent-reads-local-not-backend.md) |
