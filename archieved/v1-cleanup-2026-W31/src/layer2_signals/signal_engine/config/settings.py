...
from src.common.db import get_sqlalchemy_url  # noqa: E402
...
class Settings:
    ...
    # Database connection (PostgreSQL; connectivity is built by src.common.db)
    db_server: str
    db_user: str
    db_password: str
    ...
    def from_env(cls, env_path: Optional[str] = None) -> "Settings":
        ...
        logger.info(
            f"Settings loaded: DB_SERVER={settings.db_server}, DB_NAME={settings.db_name}"
        )
        return settings
    ...
    def get_connection_string(self) -> str:
        """
        Return the canonical PostgreSQL SQLAlchemy URL.

        Connectivity is centralised in :mod:`src.common.db`; this is provided
        for callers that want the DSN string. ``DatabaseConnection`` does not
        rely on it — it uses ``src.common.db`` directly.
        """
