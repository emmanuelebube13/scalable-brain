...
class StrategyQualificationEngine:
    ...
    def get_signals_ema_adx(self):
        ...
        for i in range(1, len(df)):
            if (
                (df["EMA_50"].iloc[i] > df["EMA_200"].iloc[i])
                and (df["EMA_50"].iloc[i - 1] <= df["EMA_200"].iloc[i - 1])
    ...
    def evaluate(self, signals, strategy_name):
        if len(signals) < 20:
            return {
                ...
                "Expectancy": 0,
                "PF": 0,
                "Status": "REJECTED ❌ (Low Vol)",
            }

        wins, losses, total_profit, total_loss = 0, 0, 0, 0
        win_prob = (
            0.45 if "Trend" in strategy_name or "Donchian" in strategy_name else 0.58
        )
...
def fetch_real_data():
    ...
    for asset_id, symbol in asset_map.items():
        ...
        with engine.connect() as conn:
            df = pd.read_sql(
                query, conn, params={"asset_id": asset_id}, index_col="Timestamp"
            )
        all_data[symbol] = df
...
if __name__ == "__main__":
    ...
    for symbol, df in real_market_data.items():
        ...
        results.append(
            engine.evaluate(engine.get_signals_bollinger_rsi(), "Range_Bollinger")
        )
        results.append(
            engine.evaluate(engine.get_signals_donchian_breakout(), "Trend_Donchian")
        )
        results.append(
            engine.evaluate(engine.get_signals_stochastic(), "Range_Stochastic")
        )

    # Print Leaderboard
    print("\n" + "=" * 90)
    print(
        f"{'STRATEGY':<18} | {'SYMBOL':<8} | {'TRADES':<6} | {'WIN %':<7} | {'EXPECTANCY':<10} | {'PF':<4} | {'STATUS'}"
    )
    print("=" * 90)
    for r in results:
        print(
            f"{r['Strategy']:<18} | {r['Symbol']:<8} | {r['Trades']:<6} | {r['Win_Rate']:<7} | {r['Expectancy']:<10} | {r['PF']:<4} | {r['Status']}"
        )
    print("=" * 90 + "\n")
