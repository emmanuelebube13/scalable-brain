...
try:
    ...
    logging.getLogger("layer4").warning(
        f"Layer 3 import failed: {e}, trying fallback..."
    )
...
LAYER3_MODELS_DIR = (
    _models_dir_env if _models_dir_env.is_absolute() else (ROOT_DIR / _models_dir_env)
)
...
LAYER3_LEGACY_PREPROCESSOR_PATH = (
    LAYER3_MODELS_DIR / "best_ml_gatekeeper_preprocessor.pkl"
)
...
# Lowercase table names (PostgreSQL folds unquoted identifiers to lowercase;
# SQLAlchemy's inspector also needs the real lowercase names).
...
def create_db_engine() -> sa.engine.Engine:
    """Create the canonical PostgreSQL + TimescaleDB SQLAlchemy engine.

    FND-004 Phase 3: was an ODBC-detected ``mssql+pyodbc`` engine; now delegates
    to :mod:`src.common.db` (``postgresql+psycopg2``, UTC session, pooled).
    """
    if not all([os.getenv("DB_SERVER"), os.getenv("DB_USER"), os.getenv("DB_PASS")]):
        raise ValueError("Missing required database credentials in environment")
    return get_engine()


class _ColumnSet(frozenset):
    """Column-name set with case-insensitive membership.

    PostgreSQL folds unquoted identifiers to lowercase, so live column names are
    lowercase while this module checks mixed-case names (e.g. ``'Is_Active'``).
    """
...
def _table_columns_ci(engine_obj, table_name: str) -> "_ColumnSet":
    """Return a case-insensitive set of a table's column names."""
    return _ColumnSet(c["name"] for c in sa.inspect(engine_obj).get_columns(table_name))


# Conflict target for fact_live_trades (UNIQUE constraint uq_livetrades_signal).
_LIVE_TRADES_CONFLICT = ("timestamp", "asset_id", "strategy_id", "signal_value")


def _upsert_live_trade(
    engine: sa.engine.Engine,
    candidate_values: Dict[str, Any],
    *,
    conflict_update: bool,
) -> bool:
    """Schema-aware idempotent upsert into fact_live_trades.

    Only columns that exist in the live table are written (the table has drifted
    from the original SQL Server schema — see CODE_MIGRATION_PHASE3.md). The
    conflict target is the ``uq_livetrades_signal`` unique key; ``created_at`` is
    set to ``now()`` on insert and ``updated_at`` (when present) on update.
    ``candidate_values`` keys are lowercase PostgreSQL column names.
    """
    present = {str(c).lower() for c in _table_columns_ci(engine, LIVE_TRADES_TABLE)}
    cols = [c for c in candidate_values if c in present]
    if not cols:
        logger.debug(
            "fact_live_trades has none of the candidate columns; skipping write"
        )
        return False

    insert_cols = list(cols)
    insert_vals = [f":{c}" for c in cols]
    if "created_at" in present and "created_at" not in insert_cols:
        insert_cols.append("created_at")
        insert_vals.append("now()")

    conflict_cols = [c for c in _LIVE_TRADES_CONFLICT if c in present]
    conflict_sql = ""
    if conflict_cols:
        if conflict_update:
            update_targets = [c for c in cols if c not in conflict_cols]
            set_parts = [f"{c} = EXCLUDED.{c}" for c in update_targets]
    ...
    query = sa.text(
        f"INSERT INTO {LIVE_TRADES_TABLE} ({', '.join(insert_cols)}) "
        f"VALUES ({', '.join(insert_vals)}) {conflict_sql}"
    )
    bind_params = {c: candidate_values[c] for c in cols}
...
def load_live_signals_with_features(
    ...
    # Check for is_active column (case-insensitive; PG folds to lowercase)
    signal_cols = _table_columns_ci(engine, SIGNALS_TABLE)
    ...
    outcome_cols = _table_columns_ci(engine, "fact_trade_outcomes")
    ...
    # Build outcome column selection dynamically (quoted mixed-case aliases).
    outcome_select_cols = []
    if "Is_Winner" in outcome_cols:
        outcome_select_cols.append('t.Is_Winner AS "Is_Winner"')
    if "R_Multiple" in outcome_cols:
        outcome_select_cols.append('t.R_Multiple AS "R_Multiple"')
    if "Holding_Bars" in outcome_cols:
        outcome_select_cols.append('t.Holding_Bars AS "Holding_Bars"')

    outcome_select = (
        ",\n        ".join(outcome_select_cols) if outcome_select_cols else ""
    )

    # Check which columns exist in the regime table
    regime_cols = _table_columns_ci(engine, REGIME_TABLE)

    # Build dynamic regime column selection (quoted mixed-case aliases).
    base_regime_cols = [
    ...
    for col in optional_regime_cols:
        if col in regime_cols:
            base_regime_cols.append(f'r.{col} AS "{col}"')
    ...
    # COMPREHENSIVE query joining all three tables. Output labels are
    # double-quoted to preserve the mixed-case DataFrame column contract.
    query = sa.text(f"""
    SELECT
        -- Core identifiers
...
def load_current_regime(
    engine: sa.engine.Engine, asset_id: int, granularity: str, timestamp: datetime
    ...
    SELECT
        ...
        ATR_Value AS "ATR_Value",
        ADX_Value AS "ADX_Value",
        Session_Volume_Z AS "Session_Volume_Z",
        Regime_Model_Version AS "Regime_Model_Version"
    FROM {REGIME_TABLE}
...
def load_model_artifact(
    ...
    if manifest:
        ...
        if LAYER3_STABLE_ALIAS.exists() and LAYER3_PREPROCESSOR_ALIAS.exists():
            ...
            LAYER3_LEGACY_MODEL_PATH.exists()
            and LAYER3_LEGACY_PREPROCESSOR_PATH.exists()
        ):
...
def fetch_live_price(
    symbol: str, granularity: str = "H1", count: int = 50
...
def compute_atr_risk_parameters(
    ...
    if direction == 1 and (stop_loss >= entry_price or take_profit <= entry_price):
        logger.error(
            f"Invalid SL/TP for BUY: entry={entry_price}, SL={stop_loss}, TP={take_profit}"
        )
        return None
    if direction == -1 and (stop_loss <= entry_price or take_profit >= entry_price):
        logger.error(
            f"Invalid SL/TP for SELL: entry={entry_price}, SL={stop_loss}, TP={take_profit}"
        )
        return None
...
def fetch_price_history(
    ...
    try:
        df = pd.read_sql(
            query, engine, params={"asset_id": asset_id, "granularity": granularity}
        )
        if len(df) < bars * 0.8:
            return None
...
def evaluate_correlation_gate(
    ...
    if new_asset_prices is None:
        logger.warning(
            f"Cannot calculate correlation: no price history for Asset_ID={signal.asset_id}"
        )
        return CorrelationResult(
            passed=True,
    ...
    for position in open_positions:
        existing_prices = fetch_price_history(
            engine,
            position["asset_id"],
            position.get("granularity", signal.granularity),
        )
...
def prepare_features_for_inference(
    signal_row: pd.Series, artifact: ModelArtifact
) -> pd.DataFrame:
    """
    ...
    """
    logger.debug(
        f"prepare_features_for_inference: Starting with {len(signal_row)} fields"
    )
    ...
    if missing:
        logger.warning(
            f"Error: {len(missing)} columns still missing after alignment: {sorted(list(missing)[:5])}..."
        )
...
def run_ml_gatekeeper(
    artifact: ModelArtifact, signal_row: pd.Series
    ...
    if (
        artifact.supported_granularities
        and granularity not in artifact.supported_granularities
    ):
        logger.warning(
...
def prepare_broker_order(
    signal: SignalContext, risk: RiskParameters, symbol_map: Dict[int, str]
) -> Dict[str, Any]:
    """
...
def execute_broker_order(
    order_params: Dict[str, Any], dry_run: bool = False
...
def write_pre_execution_log(engine: sa.engine.Engine, result: ExecutionResult) -> bool:
    """Write pre-execution state to database (schema-aware upsert)."""
    candidate_values = {
    ...
    try:
        return _upsert_live_trade(engine, candidate_values, conflict_update=True)
    except Exception as e:
...
def update_post_execution_log(
    ...
    """Update execution log with post-execution details (schema-aware).

    Only the SET targets that exist in the live table are updated. The row is
    identified by the uq_livetrades_signal key (unique), so no TOP/sub-select is
    needed. If none of the target columns exist, this is a logged no-op.
    """
    present = {str(c).lower() for c in _table_columns_ci(engine, LIVE_TRADES_TABLE)}
    ...
    if "order_id" in present:
        set_parts.append("order_id = COALESCE(:order_id, order_id)")
        bind["order_id"] = broker_order_id
    if "execution_status" in present:
        set_parts.append("execution_status = :final_status")
        bind["final_status"] = final_status
    if "veto_reason" in present:
        set_parts.append("veto_reason = COALESCE(:status_note, veto_reason)")
        bind["status_note"] = status_note
    if "updated_at" in present:
        set_parts.append("updated_at = now()")

    if not set_parts:
        logger.debug("fact_live_trades lacks post-execution columns; skipping update")
        return True
    ...
    if "execution_status" in present:
        where_parts.append("execution_status = 'PENDING'")

    query = sa.text(
        f"UPDATE {LIVE_TRADES_TABLE} SET {', '.join(set_parts)} "
        f"WHERE {' AND '.join(where_parts)}"
    )

    try:
        with engine.connect() as conn:
            conn.execute(query, bind)
            conn.commit()
        return True
    except Exception as e:
...
def log_skipped_trade(
    ...
    """Log a skipped trade with reason (schema-aware upsert)."""
    candidate_values = {
    ...
    try:
        _upsert_live_trade(engine, candidate_values, conflict_update=True)
...
class ExecutionPipeline:
    ...
    def load_symbol_map(self) -> None:
        ...
        try:
            df = pd.read_sql(
                sa.text(
                    'SELECT asset_id AS "Asset_ID", symbol AS "Symbol" FROM dim_asset'
                ),
                self.engine,
            )
    ...
    def process_signal(self, signal_row: pd.Series) -> ExecutionResult:
        ...
        logger.info(f"\n{'='*60}")
        logger.info(
            f"Processing signal: {signal_row['Symbol']} | Strategy {signal_row['Strategy_ID']}"
        )
        ...
        if pd.isna(regime_label) or regime_label == "UNKNOWN":
            ...
            log_skipped_trade(
                self.engine,
                signal_row,
                TradeDecision.SKIPPED_NO_REGIME,
        ...
        model_decision, confidence = run_ml_gatekeeper(self.model_artifact, signal_row)

        logger.info(
            f"  Decision: {model_decision.value} | Confidence: {confidence:.3f} | Threshold: {self.model_artifact.threshold:.3f}"
        )
        ...
        if risk is None:
            log_skipped_trade(
                self.engine,
                signal_row,
                TradeDecision.SKIPPED_INVALID_ATR,
        ...
        logger.info(
            f"  Entry: {risk.entry_price:.5f} | SL: {risk.stop_loss:.5f} | TP: {risk.take_profit:.5f}"
        )
        ...
        if not self.skip_correlation:
            ...
            logger.info(
                f"  Passed: {correlation_result.passed} | Score: {correlation_result.correlation_score}"
            )
            ...
            if not correlation_result.passed:
                log_skipped_trade(
                    self.engine,
                    signal_row,
                    TradeDecision.VETOED_CORRELATION,
        ...
        execution_result = ExecutionResult(
            ...
            atr_value=risk.atr_value,
            correlation_result=correlation_result,
            execution_metadata={"adx_value": signal_row.get("ADX_Value", None)},
        )
        ...
        if success:
            ...
            rr = (
                abs(
                    (risk.take_profit - risk.entry_price)
                    / (risk.entry_price - risk.stop_loss)
                )
                if (risk.entry_price - risk.stop_loss) != 0
                else 0.0
            )
            ...
            execution_result.veto_reason = (
                f"Broker execution failed: {exec_meta.get('error')}"
            )
    ...
    def run(
        ...
        logger.info(
            f"Model: {self.model_artifact.model_type} (run_id={self.model_artifact.run_id})"
        )
...
def main() -> int:
    ...
    try:
        ...
        model_artifact = load_model_artifact(
            use_stable_alias=use_stable_alias, manifest_path=manifest_path
        )
        ...
        if not results:
            logger.info(
                "Layer 4 completed with no eligible signals in the current window"
            )
        return 0
