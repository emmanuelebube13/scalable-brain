...
# Output labels are double-quoted to preserve the mixed-case column names the
# Dash callbacks reference (PostgreSQL lower-cases unquoted output labels).
QUERY = sa.text("""
SELECT
    flt.Timestamp AS "Timestamp",
    da.Symbol AS "Asset_Symbol",
    dsr.Strategy_Name AS "Strategy_Name",
    flt.Signal_Value AS "Signal_Value",
    flt.Entry_Price AS "Entry_Price",
    flt.Stop_Loss AS "Stop_Loss",
    flt.Take_Profit AS "Take_Profit",
    flt.Confidence_Score AS "Confidence_Score",
...
asset_options = (
    [{"label": i, "value": i} for i in initial_df["Asset_Symbol"].unique()]
    if not initial_df.empty
    else []
)
strategy_options = (
    [{"label": i, "value": i} for i in initial_df["Strategy_Name"].unique()]
    if not initial_df.empty
    else []
)
...
app.layout = dbc.Container(
            ...
            [
                dbc.Col(
                    dcc.Dropdown(
                        ...
                        style={
                            "color": "#000000"
                        },  # Keep text black so it's readable in the white dropdown box
                    ),
                    md=4,
                ),
                dbc.Col(
            ...
            [
                dbc.Col(
                    [
                        html.H5(
                            "Recent AI Decisions",
                            style={"color": "#A0A0A0", "margin-bottom": "15px"},
                        ),
    ...
    fluid=True,
    style={"backgroundColor": "#0A0A0A", "minHeight": "100vh", "padding": "30px"},
)
...
def update_dashboard(n, selected_assets, selected_strategies, start_date, end_date):
    ...
    if df.empty:
        return (
            [dbc.Col(html.Div("No data available", className="text-white"))],
            px.pie(),
            px.scatter(),
            [],
            [],
    ...
    if df.empty:  # Handle case where filters remove all data
        return (
            [
                dbc.Col(
                    html.Div("No data matches current filters.", className="text-white")
                )
            ],
            px.pie(),
            px.scatter(),
            ...
            dbc.Card(
                dbc.CardBody(
                    [
                        ...
                        html.H3(f"{total_signals:,}", style={"color": "#E0E0E0"}),
                    ]
                ),
                style={"backgroundColor": "#121212", "border": "1px solid #222"},
            ),
            width=4,
        ),
        dbc.Col(
                ...
                dbc.CardBody(
                    [
                        ...
                        html.H3(
                            f"{approval_rate:.1f}%",
                            style={
                                "color": "#00E676" if approval_rate > 0 else "#FF5252"
                            },
                        ),
                    ]
                ),
            ...
            dbc.Card(
                dbc.CardBody(
                    [
                        ...
                        html.H3(f"{avg_confidence:.4f}", style={"color": "#64FFDA"}),
                    ]
                ),
                style={"backgroundColor": "#121212", "border": "1px solid #222"},
            ),
    ...
    pie_fig = px.pie(
        ...
        color="Status",
        color_discrete_map={"Approved": "#00E676", "Vetoed": "#FF5252"},
    )
    ...
    scatter_fig.add_hline(
        ...
        line_dash="dash",
        line_color="#8892B0",
        annotation_text="Threshold (0.535)",
    ...
    table_df["Confidence_Score"] = table_df["Confidence_Score"].apply(
        lambda x: f"{x:.4f}"
    )
