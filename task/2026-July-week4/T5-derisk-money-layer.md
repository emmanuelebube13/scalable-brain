...
> **First action: read `task/2026-July-week4/STATE.md`.** Independent of T1–T4; parts of this task are expected to end BLOCKED on user/VM access — that is a valid outcome, record it precisely.
...
3. **Unit-confusion fixes (Agent C) — S3-002 and S3-004.** For each: read the fix doc, locate the defect in the reference copy (`OtherSystems/system-3-account-management/`) or captured code, and produce in `task/2026-July-week4/T5-fix-package/`:
...
Write to `task/2026-July-week4/deliverables/T5/` (the fix package itself stays in `T5-fix-package/`; these are the reports about it):
